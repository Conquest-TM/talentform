        ...
        var ready = function () {
            try {
                ...
                if ($$('.form-product-has-subproducts').length > 0) {
                    if (JotForm.newDefaultTheme || JotForm.isComparePaymentFormV1()) {
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    setFocusEvents: function () {
        ...
        if ((JotForm.newDefaultTheme || JotForm.isComparePaymentFormV1()) && window.paymentType === 'product' && window.FORM_MODE !== 'cardform') {
        ...
        if ((JotForm.newDefaultTheme || JotForm.isComparePaymentFormV1()) && window.paymentType === 'subscription') {
    ...
    */
    multiSelectDropdownHandler: function(onChange){
		...
		$A(options).each(function(option){
            option.observe('click', function(event){
                ...
                $A(options).each(function(opt){
                    if (opt.hasClassName('selected')){ selectedOptions.push(opt.select('input')[0].value); }
                });
                ...
                if (dropdown.hasClassName('hasSelectAll') && selectedOptions.length === 0){
                    ...
                    selectedOptions.push(options[0].select('input')[0].value);
                }
                ...
                selectedValueArea.innerText = selectedOptions.join(',');
    ...
    handleProductCategoryDropdown: function (){
        this.multiSelectDropdownHandler(function(selectedCategories){ // onChange
            ...
            var products = isCategoryTitleEnabled ? $$('.form-product-item:not([categories="non-categorized"])') : $$('.form-product-item');
            ...
            $A(products).each(function(p){
                var product = $(p);
                product.removeClassName('not-found');
                    ...
                    if (!isCategoryTitleEnabled){
                        ...
                        if (productCategories){
                            ...
                            $A(productCategories).each(function(productCategory){
                                if (selectedCategories.indexOf(productCategory) > -1){
                                    isCategoryFound = true;
                                }
                            });

                            if (!isCategoryFound){
                                product.addClassName('not-found');
                            }
                        }
                    } else {
                        if (selectedCategories.indexOf(product.getAttribute('active-category')) === -1){
                            product.addClassName('not-found');
                        }
                    }
                }
            ...
            $A(allCategoryTitles).each(function(categoryTitle){
                categoryTitle.removeClassName('not-found');

                if (selectedCategories[0] !== 'All'){
                    if (selectedCategories.indexOf(categoryTitle.getAttribute('category')) === -1){
                        categoryTitle.addClassName('not-found');
                    } else {
                        categoryTitle.removeClassName('not-found');
                    }
                }
